...
while True:
    ...
    if choice==1:
        ...
        searchName=input("검색할 학생의 이름을 입력하세요(0.취소)>>")
        chk=0
        if searchName==0:
            break
        for s_dic in students:
            if s_dic["name"] ==searchName:
                break
            chk+=1
        print("찾고자하는 학생의 위치:",chk)
        
        if chk ==len(students):
            print('{}학생을 찾을수 없습니다.다시입력하세요'.format(searchName))     
        else:
            print('{}학생을 찾았습니다.'.format(searchName))
            while True:
                print("수정할 과목 선택") 
                print("1.국어\t2.영어\t3.수학")
                c_title=int(input("수정하려는 과목을 택하세요.(0.취소)")) 
                if c_title == 1:
                    c_1="kor"
    elif choice==4:
