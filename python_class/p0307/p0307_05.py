'''
words =[{"airplane":"비행기","apple":"사과",
         "bakery":"빵집","banana":"바나나",
         "bank":"은행","bean":"콩","bicycle":"자전거",
         "boat":"보트","bowl":"보트","bus":"버스"},
        {"run":"달리다","walk" : "걷다","sit" : "앉다",
         "stand":"서다","sleep":"자다","read":"읽다",
         "look":"보다","do":"하다","feel":"느끼다","go":"가다"},
         {"accumulated":"누적된","additional":"추가적인",
         "dequate":"적당한","administrative":"관리의",
    ...
    if chk == "1":
        ...
        for key in words[choice]:
            ...
            if answer ==words[choice][key]:
                    ...
                    print("오답입니다.정답은:{}".format(words[choice][key]))
                
            print("정답개수는:{}, 오답개수:{}".format(count,words[choice])-count) 
    else:
...
def w_quiz(choice):
    ...
    if chk == "1":
        ...
        print("정답개수 : {}, 오답 : {} ".format(count,len(words[choice])-count))
    else:
        print("퀴즈를 취소하셨습니다. 메뉴로 돌아갑니다.")
while True:
    print("[ 영단어 맞추기 프로그램 ]")
